...
describe("web security", () => {
  ...
  it("allows the internal health check only with its secret token", async () => {
    ...
    expect(allowed.json()).toMatchObject({ status: "ok", version: "0.8.50" });
  });
...
describe("automations", () => {
  it("creates a device-state automation with a room, optional condition and toggle action", async () => {
    ...
    expect(response.statusCode).toBe(201);
    expect(create).toHaveBeenCalledWith(expect.objectContaining({ roomId: room.id, triggerValue: true, additionalTriggers: [{ deviceId: "wall-switch", stateKey: "on", value: true }], conditionValue: false, action: "toggle" }));
  });
...
describe("disaster recovery backup API", () => {
  it("exports a password encrypted full recovery backup", async () => {
    vi.mocked(createDisasterRecoveryBackup).mockResolvedValueOnce({
      format: "salta-disaster-recovery-backup", formatVersion: 1, saltaVersion: "0.8.50", createdAt: "2026-08-14T07:00:00.000Z",
    ...
    expect(createDisasterRecoveryBackup).toHaveBeenCalledWith("0.8.50", "correct horse battery staple");
  });
