...
describe("web security", () => {
  ...
  it("allows the internal health check only with its secret token", async () => {
    ...
    expect(allowed.json()).toMatchObject({ status: "ok", version: "0.8.69" });
  });
...
describe("disaster recovery backup API", () => {
  it("exports a password encrypted full recovery backup", async () => {
    vi.mocked(createDisasterRecoveryBackup).mockResolvedValueOnce({
      format: "salta-disaster-recovery-backup", formatVersion: 1, saltaVersion: "0.8.69", createdAt: "2026-08-14T07:00:00.000Z",
    ...
    expect(createDisasterRecoveryBackup).toHaveBeenCalledWith("0.8.69", "correct horse battery staple");
  });
