...
describe("HomeKit settings frontend", () => {
  it("provides a dedicated HomeKit settings panel with runtime and pairing controls", () => {
    ...
    for (const id of ["homeKitEnabled", "homeKitName", "homeKitNetworkInterface", "homeKitPairingState", "homeKitPairingQr", "homeKitPairingCode", "homeKitResetButton"]) {
  ...
  it("keeps pairing information compact and selectable", () => {
    expect(cssRuleContains(styles, ".homekit-runtime-grid", "grid-template-columns:repeat(auto-fit,minmax(150px,1fr))")).toBe(true);
