  ...
  async add(host: string, username = "", password = "", name?: string, roomId?: string, room?: string, credentialMode: "inherit" | "custom" | "none" = "inherit"): Promise<Device[]> {
    ...
    for (const [index, result] of results.entries()) {
      ...
      const device: Device = {
        ...
        hidden: existing?.hidden ?? result.device.hidden,
        presentationType: existing?.presentationType ?? result.device.presentationType ?? "auto",
        credentialMode,
  ...
  async refresh(device: Device): Promise<Device> {
    ...
    try {
      ...
      const next: Device = {
        ...
        presentationType: device.presentationType ?? "auto",
