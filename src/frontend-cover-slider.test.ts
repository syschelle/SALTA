...
describe("window-covering position slider", () => {
  ...
  it("sends setTargetPosition and keeps active slider input stable during live refresh", () => {
    ...
    expect(source).toContain("if(!activeCoverSliderId&&!activeBrightnessSliderId&&!activeTemperatureSliderId&&!activeHueColorTemperatureId&&!activeHueColorId)renderDevices()");
