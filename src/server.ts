...
export function buildServer(registry: DeviceRegistry, shellyAdapter: ShellyAdapter, phosconAdapter: PhosconAdapter, openCcuAdapter: OpenCcuAdapter) {
  ...
  app.get("/internal/health", async () => ({ status: "ok", name: "SALTA", version: "0.7.14" }));

  app.get("/api/health", async () => ({ status: "ok", name: "SALTA", version: "0.7.14", time: new Date().toISOString() }));
