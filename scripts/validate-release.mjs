...
const virtualAdapterSource = read("src/virtual-adapter.ts");
...
if ((databaseSource.match(/DELETE FROM commands WHERE created_at < now\(\) - interval '90 days'/g) ?? []).length !== 2) fail("Command retention must prune entries older than 90 days at startup and during runtime");
if ((databaseSource.match(/ORDER BY created_at DESC, id DESC OFFSET 10000/g) ?? []).length !== 2) fail("Command retention must keep at most the 10000 newest entries at startup and during runtime");
if (!databaseSource.includes("export async function pruneCommandHistory(): Promise<void>")) fail("Command retention helper is missing");
if (!serverSource.includes("await pruneCommandHistory().catch(() => undefined)")) fail("New API commands do not trigger bounded command-history cleanup");
if (!read("src/command-retention.test.ts").includes("prunes command history to 90 days and at most 10000 newest rows")) fail("Command retention regression coverage is missing");
...
if (!phosconAdapterSource.includes("processedButtonEventSignatures") || !phosconAdapterSource.includes("pendingButtonEventSignatures")) fail("Phoscon button-event cross-transport deduplication is missing");
if (!phosconAdapterSource.includes("this.claimButtonEvent(resourceId, eventValue, discoveredButtonUpdated, receivedAt)") || !phosconAdapterSource.includes('buttonEventTransport: "reconcile"')) fail("Phoscon reconcile does not recover missed button events");
if (!phosconAdapterSource.includes("this.commitButtonEvent(claimedSignature)") || !phosconAdapterSource.includes("this.releaseButtonEvent(claimedSignature)")) fail("Phoscon button-event claim lifecycle is incomplete");
if (!read("src/phoscon-button-race.test.ts").includes("emits a button event when normal reconcile discovers a timestamp missed by realtime polling") || !read("src/phoscon-button-race.test.ts").includes("does not emit twice when reconcile and fallback poll observe the same event concurrently")) fail("Phoscon button-event race regression coverage is missing");
if (!phosconAdapterSource.includes("processedButtonEventSignatures") || !phosconAdapterSource.includes("pendingButtonEventSignatures")) fail("Phoscon button-event cross-transport deduplication is missing");
if (!phosconAdapterSource.includes("this.claimButtonEvent(resourceId, eventValue, discoveredButtonUpdated, receivedAt)") || !phosconAdapterSource.includes('buttonEventTransport: "reconcile"')) fail("Phoscon reconcile does not recover missed button events");
if (!phosconAdapterSource.includes("this.commitButtonEvent(claimedSignature)") || !phosconAdapterSource.includes("this.releaseButtonEvent(claimedSignature)")) fail("Phoscon button-event claim lifecycle is incomplete");
if (!read("src/phoscon-button-race.test.ts").includes("emits a button event when normal reconcile discovers a timestamp missed by realtime polling") || !read("src/phoscon-button-race.test.ts").includes("does not emit twice when reconcile and fallback poll observe the same event concurrently")) fail("Phoscon button-event race regression coverage is missing");
...
if (publicIndex.includes("Virtuelle Trigger-Schalter können sich sicher selbst zurücksetzen") || publicIndex.includes("automation-self-reset-hint")) fail("Automation editor still contains the removed virtual self-reset hint");
if (!publicIndex.includes('<option value="button" selected>Taster (Impuls)</option>')) fail("Virtual-device editor does not offer the momentary button type");
if (!serverSource.includes('type: z.enum(["switch", "button"]).default("switch")') || !serverSource.includes("virtualAdapter.createButton")) fail("Virtual-device API does not create momentary buttons");
if (!serverSource.includes('virtualType: z.enum(["switch", "button"]).optional()') || !serverSource.includes("virtualAdapter.updateKind")) fail("Existing virtual devices cannot be switched between persistent switch and momentary button mode");
if (!virtualAdapterSource.includes("MOMENTARY_BUTTON_RESET_MS = 500") || !virtualAdapterSource.includes('virtualType: kind') || !virtualAdapterSource.includes("scheduleMomentaryReset") || !virtualAdapterSource.includes("async updateKind(")) fail("Virtual momentary-button create/convert/auto-reset support is missing");
if (!virtualFrontend.includes("function isVirtualButton(d)") || !virtualFrontend.includes("<span>Drücken</span>") || !publicIndex.includes('id="deviceVirtualType"')) fail("Virtual momentary-button frontend presentation/configuration is missing");
...
const directMd5Calls = presenceSource.match(/createHash\(["']md5["']\)/gi) ?? [];
if (directMd5Calls.length !== 0) fail(`FRITZ!Box Presence must not contain direct literal MD5 createHash calls, found ${directMd5Calls.length}`);
if ((presenceSource.match(/digestHash\("MD5"/g) ?? []).length !== 2) fail("FRITZ!Box SOAP content authentication must retain exactly two protocol MD5 digest-helper calls");
if (!presenceSource.includes('normalized === "MD5" ? "md5" : undefined') || !presenceSource.includes('return createHash(nodeAlgorithm).update(value).digest("hex")')) fail("FRITZ!Box protocol digest helper no longer supports its documented MD5 interoperability path");
if (/codeql\[js\/(?:insufficient-password-hash|weak-cryptographic-algorithm)\]|lgtm\[js\/weak-cryptographic-algorithm\]/.test(presenceSource)) fail("Stale FRITZ!Box MD5 source suppressions remain after centralizing the protocol digest helper");
if (!presenceSource.includes("protocol digest helper used for HTTP Digest") || !presenceSource.includes("SALTA never stores passwords with MD5")) fail("FRITZ!Box protocol-mandated MD5 rationale is missing");
if (!securitySource.includes("Protocol-mandated MD5 in FRITZ!Box authentication") || !securitySource.includes("This is not password storage")) fail("SECURITY.md does not document the scoped FRITZ!Box protocol MD5 exception");
...
if (!publicIndex.includes('id="overviewFavoritesSection"') || !publicIndex.includes('id="overviewFavoritesGrid"') || !publicIndex.includes('id="deviceFavorite" type="checkbox"')) fail("Overview favorites UI is missing");
if (!databaseSource.includes("CREATE TABLE IF NOT EXISTS device_favorites") || !databaseSource.includes('COALESCE(f.favorite,false) as favorite')) fail("Device favorites persistence is missing");
if (!virtualFrontend.includes("function renderOverviewFavorites()") || !virtualFrontend.includes("deviceCard(device,false,'favorite')")) fail("Overview favorites rendering is missing");
if (!configurationBackupSource.includes("device_favorites: backupRows().optional()") || !configurationBackupSource.includes("device_favorites: backup.data.device_favorites ?? []")) fail("Device favorites backup compatibility is missing");
