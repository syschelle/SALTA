...
In the production image stack, SALTA uses host networking for HomeKit mDNS/HAP discovery. PostgreSQL remains on Docker's normal bridge network and is published only to host loopback as `127.0.0.1:${POSTGRES_HOST_PORT:-5433}:5432`. This lets host-network SALTA connect through `127.0.0.1` without exposing PostgreSQL on a LAN-facing address. Do not change the PostgreSQL host binding to `0.0.0.0` or a LAN address unless external database access is deliberately required and separately secured. The production Compose intentionally defines no custom `frontend`, `backend` or `internal: true` networks.
