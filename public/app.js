...
const sourceLabels={shelly:'Shelly',phoscon:'Zigbee',openccu:'HomeMatic'};
...
function renderOverviewDevices(){const knownRoomIds=new Set(rooms.map(room=>room.id));const assigned=all.filter(device=>device.roomId&&knownRoomIds.has(device.roomId));if(!assigned.length){overviewDeviceGrid.innerHTML='<article class="empty-state"><h3>Noch keine Geräte zugeordnet</h3><p class="muted">Ordne Geräte einem Raum zu, damit sie hier in der Übersicht erscheinen.</p></article>';return}const groups=rooms.map(room=>({id:room.id,name:room.name,icon:room.icon||'home-outline',devices:assigned.filter(device=>device.roomId===room.id)})).filter(group=>group.devices.length);overviewDeviceGrid.innerHTML=groups.map(group=>deviceRoomGroup(group,true,'overview')).join('')}
...
function renderDevices(){renderOverviewDevices();renderDeviceGrid('shelly',deviceGrid,filter,roomFilter);renderDeviceGrid('phoscon',zigbeeGrid,zigbeeFilter,zigbeeRoomFilter);renderDeviceGrid('openccu',openCcuGrid,openCcuFilter,openCcuRoomFilter)}
