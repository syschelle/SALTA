...
function homeKitSupportedDevice(d){const type=resolvedPresentationType(d);if(type==='windowCovering')return d.capabilities.includes('setTargetPosition');return ['switch','outlet','light','fan'].includes(type)&&d.capabilities.includes('turnOn')&&d.capabilities.includes('turnOff')}
...
if(d.source==='virtual')add('Virtueller Typ',adapter.virtualType||d.type);
...
function openDevice(id){
  ...
  const visualType=resolvedPresentationType(selectedDevice);const lastSeenMeta=shelly?`<span class="device-dialog-last-seen">Zuletzt gesehen: ${escapeHtml(deviceInfoTimestamp(selectedDevice.lastSeen))}</span>`:'';
  ...
  const configurable=!virtual&&supportsPresentationOverride(selectedDevice);devicePresentationSection.hidden=!configurable;devicePresentationType.value=selectedDevice.presentationType||'auto';devicePresentationType.options[0].textContent=`Automatisch (${typeLabels[selectedDevice.type]||selectedDevice.type})`;
...
async function saveDeviceConfig(){
  ...
  const config={name,roomId:deviceRoom.value||null,presentationType,homekitEnabled:homekitSupported&&deviceHomeKitEnabled.checked,homekitName:deviceHomeKitName.value.trim()||null,homekitUseSaltaRoom:useSaltaRoom,homekitRoomId:useSaltaRoom?null:(deviceHomeKitRoom.value||null)};
