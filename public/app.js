let all=[],rooms=[],selectedDevice=null,shellySettingsStatus=null,phosconSettingsStatus=null,openCcuSettingsStatus=null,editingRoomId=null,liveRefreshInFlight=false,activeCoverSliderId=null,activeBrightnessSliderId=null,csrfToken="";
...
const pages=['overview','shelly','zigbee','openccu','rooms','settings'];
...
async function reconcileOpenCcu(){try{await api('/api/adapters/openccu/reconcile',{method:'POST'});await load();notify('HomeMatic-Synchronisierung abgeschlossen.')}catch(error){notify(friendlyOpenCcuError(error),true)}}
...
function friendlyOpenCcuError(error){const rawCode=String(error?.code||'');const code=rawCode.split(':',1)[0];const messages={OPENCCU_URL_REQUIRED:'Trage die Adresse deiner OpenCCU-Instanz ein.',OPENCCU_URL_INVALID:'Die OpenCCU-Adresse ist ungültig. Beispiel: http://192.168.178.30',OPENCCU_CREDENTIALS_REQUIRED:'Trage OpenCCU-Benutzername und Passwort ein.',OPENCCU_NOT_CONFIGURED:'Verbinde zuerst unter Einstellungen eine OpenCCU-Instanz.',OPENCCU_AUTHENTICATION_FAILED:'Benutzername oder Passwort wurden von OpenCCU abgelehnt.',OPENCCU_UNREACHABLE:'Die OpenCCU-Instanz ist unter dieser Adresse nicht erreichbar.',OPENCCU_TIMEOUT:'Die OpenCCU-Instanz antwortet nicht rechtzeitig.',OPENCCU_TLS_ERROR:'Das HTTPS-Zertifikat der OpenCCU-Instanz kann nicht verifiziert werden.',OPENCCU_INVALID_RESPONSE:'OpenCCU hat keine gültige JSON-RPC-Antwort geliefert.',ENCRYPTION_KEY_MISMATCH:'Das gespeicherte OpenCCU-Passwort kann mit dem aktuellen SALTA_ENCRYPTION_KEY nicht entschlüsselt werden.',OPENCCU_REQUEST_FAILED:'Die Verbindung zu OpenCCU konnte nicht hergestellt werden.'};if(rawCode.startsWith('OPENCCU_API_ERROR:'))return error?.message||'OpenCCU hat einen JSON-RPC-Fehler gemeldet.';return messages[code]||error?.message||'Die OpenCCU-Anfrage ist fehlgeschlagen.'}
function renderOpenCcuConnectionNotice(){if(!openCcuSettingsStatus)return;const gateway=openCcuSettingsStatus.gateway||{};if(gateway.connected){openCcuConnectionNotice.hidden=true;openCcuConnectionNotice.textContent='';return}openCcuConnectionNotice.hidden=false;openCcuConnectionNotice.textContent=openCcuSettingsStatus.passwordConfigured?(gateway.lastError?`OpenCCU ist derzeit nicht erreichbar: ${friendlyOpenCcuError({code:gateway.lastError})}`:'OpenCCU ist konfiguriert, aber derzeit nicht verbunden.'):'Verbinde zuerst unter Einstellungen eine OpenCCU-Instanz.'}
function renderOpenCcuGatewayStatus(settings){const gateway=settings?.gateway||{};const connected=Boolean(gateway.connected);openCcuGatewayStatus.className=`gateway-status ${connected?'connected':'disconnected'}`;const details=connected?[gateway.interfaces?.length?gateway.interfaces.join(', '):'',`${Number(gateway.devices||0)} Geräte`,gateway.lastSync?`Synchronisiert ${new Date(gateway.lastSync).toLocaleString()}`:''].filter(Boolean).join(' · '):(gateway.lastError?friendlyOpenCcuError({code:gateway.lastError}):'Keine aktive Verbindung.');openCcuGatewayStatus.innerHTML=`<span class="gateway-status-dot" aria-hidden="true"></span><div><strong>${connected?'Verbunden':'Nicht verbunden'}</strong><small>${escapeHtml(details)}</small></div>`;openCcuDisconnectButton.hidden=!settings?.passwordConfigured}
async function loadOpenCcuSettings(){const settings=await api('/api/settings/openccu');openCcuSettingsStatus=settings;openCcuBaseUrl.value=settings.baseUrl||'';openCcuUsername.value=settings.username||'';openCcuPassword.value='';openCcuPasswordState.textContent=settings.passwordConfigured?(settings.encryptionStatus==='invalid'?'Das gespeicherte Passwort kann nicht entschlüsselt werden. Gib es vollständig neu ein.':'Ein Passwort ist verschlüsselt gespeichert. Leer lassen, um es beizubehalten.'):'Noch kein Passwort gespeichert.';openCcuCredentialWarning.hidden=settings.encryptionStatus!=='invalid';openCcuCredentialWarning.textContent=settings.encryptionStatus==='invalid'?'Der aktuelle SALTA_ENCRYPTION_KEY passt nicht zum gespeicherten OpenCCU-Passwort. Gib das Passwort neu ein.':'';renderOpenCcuGatewayStatus(settings);renderOpenCcuConnectionNotice();return settings}
async function saveOpenCcu(){try{const settings=await api('/api/settings/openccu',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({baseUrl:openCcuBaseUrl.value.trim(),username:openCcuUsername.value.trim(),password:openCcuPassword.value||undefined})});openCcuSettingsStatus=settings;await load();await loadOpenCcuSettings();notify('OpenCCU-Verbindung wurde gespeichert und geprüft.')}catch(error){notify(friendlyOpenCcuError(error),true)}}
async function disconnectOpenCcu(){if(!confirm('OpenCCU-Verbindung trennen? Die synchronisierten HomeMatic-Geräte werden aus SALTA entfernt, aber nicht aus OpenCCU gelöscht.'))return;await api('/api/settings/openccu',{method:'DELETE'});openCcuSettingsStatus=null;await load();await loadOpenCcuSettings();notify('OpenCCU-Verbindung wurde getrennt.')}
