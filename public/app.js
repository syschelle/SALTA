...
function deviceCard(d){const visualType=resolvedPresentationType(d);const stateVisual=['switch','light'].includes(visualType);const stateKnown=stateVisual&&d.reachable&&typeof d.state?.on==='boolean';const stateClass=stateKnown?(d.state.on?' device-state-on':' device-state-off'):'';const stateLabel=stateKnown?`, ${d.state.on?'An':'Aus'}`:'';const values=displayedState(d).filter(([key])=>!(stateVisual&&key==='on'));const model=d.model?` · ${escapeHtml(d.model)}`:'';const channel=d.profile==='switch'&&d.channelCount>1?` · Kanal ${Number(d.componentId||0)+1}`:'';return `<article class="device ${d.reachable?'':'offline'}${stateClass}"${stateKnown?` aria-label="${escapeHtml(d.name)}${stateLabel}"`:''}><div class="device-top"><div class="icon">${iconMarkup(icons[visualType]||'help-circle-outline')}</div><div class="dot"></div></div><h3>${escapeHtml(d.name)}</h3><div class="meta">${escapeHtml(d.room||'Nicht zugeordnet')} · ${escapeHtml(typeLabels[visualType]||visualType)}${channel}${model}</div>${values.length?`<div class="values">${values.map(([k,v])=>`<div class="value"><b>${fmt(k,v)}</b><small>${labels[k]||k}</small></div>`).join('')}</div>`:''}${coverControl(d)}<div class="actions">${actions(d)}</div></article>`}
