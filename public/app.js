let all=[],rooms=[],systemLogs=[],selectedDevice=null,shellySettingsStatus=null,phosconSettingsStatus=null,openCcuSettingsStatus=null,editingRoomId=null,liveRefreshInFlight=false,activeCoverSliderId=null,activeBrightnessSliderId=null,activeTemperatureSliderId=null,csrfToken="";
...
const pages=['overview','shelly','zigbee','openccu','rooms','logs','settings'];
...
function renderFilters(){fillRoomFilter(roomFilter);fillRoomFilter(zigbeeRoomFilter);fillRoomFilter(openCcuRoomFilter)}
...
function thermostatModeControl(d){if(!d.capabilities.includes('setThermostatMode'))return '';const current=thermostatModeValue(d);const disabled=d.reachable?'':' disabled';const options=[['off','Aus','power-off'],['manual','Hand','hand-back-right-outline'],['auto','Automatik','calendar-clock']];return `<div class="thermostat-mode-control"><div class="thermostat-mode-head"><span>Betriebsart</span><strong>${current?fmt('controlMode',current):'–'}</strong></div><div class="thermostat-mode-buttons" role="group" aria-label="Betriebsart von ${escapeHtml(d.name)}">${options.map(([value,label,icon])=>`<button type="button" class="${current===value?'active':''}" aria-pressed="${current===value}" onclick="setThermostatMode('${d.id}','${value}')"${disabled}>${iconMarkup(icon)}<span>${label}</span></button>`).join('')}</div></div>`}
function targetTemperatureControl(d){if(!d.capabilities.includes('setTargetTemperature'))return '';const range=thermostatRange(d);const value=targetTemperatureValue(d);const disabled=d.reachable?'':' disabled';return `<div class="temperature-control"><div class="temperature-control-head"><label for="target-temperature-${d.id}">Solltemperatur</label><output id="target-temperature-output-${d.id}" for="target-temperature-${d.id}">${fmt('targetTemperature',value)}</output></div><input id="target-temperature-${d.id}" type="range" min="${range.min}" max="${range.max}" step="${range.step}" value="${value}" onpointerdown="activeTemperatureSliderId='${d.id}'" onpointerup="activeTemperatureSliderId=null" onblur="activeTemperatureSliderId=null" oninput="previewTargetTemperature('${d.id}',this.value)" onchange="setTargetTemperature('${d.id}',this.value)" aria-label="Solltemperatur von ${escapeHtml(d.name)}"${disabled}></div>`}
function previewTargetTemperature(id,value){activeTemperatureSliderId=id;const output=document.getElementById(`target-temperature-output-${id}`);if(output)output.textContent=fmt('targetTemperature',Number(value))}
...
function renderDevices(){renderDeviceGrid('shelly',deviceGrid,filter,roomFilter);renderDeviceGrid('phoscon',zigbeeGrid,zigbeeFilter,zigbeeRoomFilter);renderDeviceGrid('openccu',openCcuGrid,openCcuFilter,openCcuRoomFilter)}
...
function renderSystemLogs(){if(!systemLogs.length){systemLogList.innerHTML='<article class="empty-state"><h3>Keine Protokolleinträge</h3><p class="muted">Für die gewählten Filter sind keine Ereignisse vorhanden.</p></article>';return}systemLogList.innerHTML=systemLogs.map(entry=>{const details=readableLogDetails(entry.details);return `<article class="system-log-entry ${escapeHtml(entry.level)}"><div class="system-log-meta"><strong>${escapeHtml(entry.level==='error'?'Fehler':entry.level==='warning'?'Warnung':'Info')}</strong><span>${escapeHtml(entry.source)}</span><time>${new Date(entry.createdAt).toLocaleString()}</time></div><div class="system-log-message"><strong>${escapeHtml(entry.message)}</strong><small>${escapeHtml(entry.code||'')}</small>${details?`<div class="system-log-details">${escapeHtml(details)}</div>`:''}</div></article>`}).join('')}
async function loadSystemLogs(){const query=new URLSearchParams({limit:'200'});if(logSourceFilter.value)query.set('source',logSourceFilter.value);if(logLevelFilter.value)query.set('level',logLevelFilter.value);const result=await api(`/api/logs?${query}`);systemLogs=result.entries||[];renderSystemLogs()}
