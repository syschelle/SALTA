...
function applyTheme(value,{persist=false,announce=false}={}){const theme=normalizeTheme(value);document.documentElement.dataset.theme=theme;document.documentElement.style.colorScheme=theme;applyAppearancePalette(theme);if(persist)writeThemeCookie(theme);updateThemeToggle(theme);if(announce)notify(theme==='dark'?'Dunkles Theme aktiviert.':'Helles Theme aktiviert.')}
...
function overviewFavoriteDevices(){return all.filter(device=>Boolean(device.favorite)&&device.source!=='presence'&&device.source!=='system'&&!(device.source==='phoscon'&&device.hidden)).sort((a,b)=>String(a.room||'').localeCompare(String(b.room||''),'de',{sensitivity:'base'})||String(a.name||'').localeCompare(String(b.name||''),'de',{sensitivity:'base'}))}
...
function renderHueGatewayStatus(settings){const gateway=settings?.gateway||{};const connected=Boolean(gateway.connected);hueGatewayStatus.className=`gateway-status ${connected?'connected':'disconnected'}`;const realtime=gateway.realtimeConnected?'Realtime: Eventstream verbunden':'Realtime: nicht verbunden';const details=connected?[gateway.name||'Hue Bridge',gateway.bridgeId?`Bridge ${gateway.bridgeId}`:'',gateway.softwareVersion?`Firmware ${gateway.softwareVersion}`:'',gateway.apiVersion?`API ${gateway.apiVersion}`:'',realtime,gateway.realtimeLastEvent?`Letztes Event ${new Date(gateway.realtimeLastEvent).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`:''].filter(Boolean).join(' · '):(gateway.lastError?friendlyHueError({code:gateway.lastError}):'Keine aktive Verbindung.');hueGatewayStatus.innerHTML=`<span class="gateway-status-dot" aria-hidden="true"></span><div><strong>${connected?'Verbunden':'Nicht verbunden'}</strong><small>${escapeHtml(details)}</small></div>`;hueDisconnectButton.hidden=!settings?.applicationKeyConfigured}
...
initializeTheme();
initializeSession().then(async()=>{await loadAppearanceSettings().catch(()=>undefined);navigate();load();setInterval(refreshLiveData,5000)}).catch(()=>undefined);
