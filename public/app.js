...
function thermostatRange(d){const metadata=d.adapterData||{};const min=Number(metadata.targetTemperatureMin??5);const max=Number(metadata.targetTemperatureMax??30);const step=Number(metadata.targetTemperatureStep??0.5);return {min:Number.isFinite(min)?min:5,max:Number.isFinite(max)?max:30,step:Number.isFinite(step)&&step>0?step:0.5}}
