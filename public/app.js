...
const sourceLabels={shelly:'Shelly',phoscon:'Zigbee',openccu:'HomeMatic',virtual:'Virtuell'};
...
function openDevice(id){selectedDevice=all.find(d=>d.id===id);if(!selectedDevice)return;const shelly=selectedDevice.source==='shelly';const zigbee=selectedDevice.source==='phoscon';const openccu=selectedDevice.source==='openccu';const virtual=selectedDevice.source==='virtual';deviceDialogTitle.textContent=selectedDevice.name;deviceDialogDescription.textContent=shelly?'Name, Raum, Gerätefunktion und Shelly-Zugangsdaten konfigurieren.':zigbee?'Name, Raum, Sichtbarkeit und lokale Darstellung des Zigbee-Geräts konfigurieren.':openccu?'Name, Raum und lokale Darstellung des HomeMatic-Geräts konfigurieren.':'Name und Raum des virtuellen SALTA-Geräts konfigurieren.';deviceName.value=selectedDevice.name;deviceRoom.innerHTML='<option value="">Nicht zugeordnet</option>'+rooms.map(r=>`<option value="${r.id}">${escapeHtml(r.name)}</option>`).join('');deviceRoom.value=selectedDevice.roomId||'';const configurable=!virtual&&supportsPresentationOverride(selectedDevice);devicePresentationSection.hidden=!configurable;devicePresentationType.value=selectedDevice.presentationType||'auto';devicePresentationType.options[0].textContent=`Automatisch (${typeLabels[selectedDevice.type]||selectedDevice.type})`;devicePresentationHint.textContent=shelly?'Die Auswahl ändert nur die logische Verwendung, nicht die physische Shelly-Konfiguration.':openccu?'Die Auswahl ändert nur die Darstellung in SALTA; das HomeMatic-Gerät in OpenCCU bleibt unverändert.':'Die Auswahl ändert nur die Darstellung in SALTA; die Zigbee-Ressource in Phoscon bleibt unverändert.';deviceVisibilitySection.hidden=!zigbee;deviceHidden.checked=Boolean(selectedDevice.hidden);deviceCredentialSection.hidden=!shelly;credentialMode.value=selectedDevice.credentialMode||'inherit';deviceUsername.value=selectedDevice.credentialUsername||'';devicePassword.value='';deviceDeleteSection.hidden=!(shelly||virtual);if(!deviceDeleteSection.hidden){deviceDeleteDescription.textContent=virtual?'Der virtuelle Schalter wird aus SALTA entfernt und verschwindet automatisch aus der HomeKit-Bridge.':'Das Shelly-Gerät selbst wird nicht zurückgesetzt oder ausgeschaltet. Es kann später erneut hinzugefügt werden.';deleteDeviceButton.textContent=virtual?'Virtuelles Gerät löschen':'Gerät aus SALTA löschen'}toggleDeviceCredentials();deviceDialog.showModal()}
