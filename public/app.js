...
function openDevice(id){selectedDevice=all.find(d=>d.id===id);if(!selectedDevice)return;const shelly=selectedDevice.source==='shelly';deviceDialogTitle.textContent=selectedDevice.name;deviceDialogDescription.textContent=shelly?'Name, Raum, Gerätefunktion und Shelly-Zugangsdaten konfigurieren.':'Name, Raum und lokale Darstellung des Zigbee-Geräts konfigurieren.';deviceName.value=selectedDevice.name;deviceRoom.innerHTML='<option value="">Nicht zugeordnet</option>'+rooms.map(r=>`<option value="${r.id}">${escapeHtml(r.name)}</option>`).join('');deviceRoom.value=selectedDevice.roomId||'';const configurable=supportsPresentationOverride(selectedDevice);devicePresentationSection.hidden=!configurable;devicePresentationType.value=selectedDevice.presentationType||'auto';devicePresentationType.options[0].textContent=`Automatisch (${typeLabels[selectedDevice.type]||selectedDevice.type})`;devicePresentationHint.textContent=shelly?'Die Auswahl ändert nur die logische Verwendung, nicht die physische Shelly-Konfiguration.':'Die Auswahl ändert nur die Darstellung in SALTA; die Zigbee-Ressource in Phoscon bleibt unverändert.';deviceCredentialSection.hidden=!shelly;credentialMode.value=selectedDevice.credentialMode||'inherit';deviceUsername.value=selectedDevice.credentialUsername||'';devicePassword.value='';deviceDeleteSection.hidden=!shelly;toggleDeviceCredentials();deviceDialog.showModal()}
async function saveDeviceConfig(){if(!selectedDevice)return;const name=deviceName.value.trim();if(!name){deviceName.focus();return}const presentationType=devicePresentationSection.hidden?(selectedDevice.presentationType||'auto'):devicePresentationType.value;await api(`/api/devices/${encodeURIComponent(selectedDevice.id)}/config`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({name,roomId:deviceRoom.value||null,presentationType})});if(selectedDevice.source==='shelly')await api(`/api/devices/${encodeURIComponent(selectedDevice.id)}/credentials`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({credentialMode:credentialMode.value,username:deviceUsername.value||undefined,password:devicePassword.value||undefined})});deviceDialog.close();await load();notify('Gerätekonfiguration gespeichert.')}
