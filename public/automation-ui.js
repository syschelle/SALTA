...
async function loadAutomations(){const payload=await api('/api/automations');automationRules=payload?.automations||[];renderAutomations();updateAutomationFormOptions()}
function resetAutomationForm(){editingAutomationId=null;automationElements.form.reset();clearAutomationDeviceSearches();automationElements.enabled.checked=true;automationElements.conditionEnabled.checked=false;automationElements.conditionFields.hidden=true;automationElements.title.textContent='Automation hinzufügen';automationElements.save.textContent='Automation speichern';automationElements.cancel.hidden=true;updateAutomationFormOptions()}
