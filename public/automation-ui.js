...
const automationActionLabels={turnOn:'An',turnOff:'Aus',toggle:'Toggle',open:'Öffnen',close:'Schließen',thermostatOff:'Thermostat Aus',thermostatAuto:'Thermostat Automatik',thermostatManual:'Thermostat Manuell',setTargetTemperature:'Solltemperatur setzen'};
...
function updateAutomationFormOptions(values={}){
  ...
  fillAutomationSelect(automationElements.actionDevice,automationActionDevices().filter(device=>automationTargetDeviceAllowed(device,new Set([...additionalActionIds].filter(id=>id!==actionSelected)))),actionSelected,'Zielgerät wählen',automationElements.actionSearch?.value,automationElements.actionCount);
...
function renderAutomationAdditionalActions(){
  ...
  for(const target of automationAdditionalActions){
    ...
    fillAutomationSelect(deviceSelect,candidates,target.deviceId,'Zielgerät wählen',target.query,count);
...
function addAutomationAdditionalAction(){
  if(automationAdditionalActions.length>=7){notify('Maximal acht Zielgeräte pro Automation.',true);return}
  ...
  if(!first){notify('Kein weiteres geeignetes Zielgerät verfügbar.',true);return}
...
function searchAutomationAdditionalAction(id,query){const target=automationAdditionalActions.find(item=>item.id===id);if(!target)return;target.query=query;const actionIds=automationAllActionDeviceIds(id);const candidates=automationActionDevices().filter(device=>automationTargetDeviceAllowed(device,actionIds));const select=document.getElementById(`automationExtraActionDevice-${id}`);const count=document.getElementById(`automationExtraActionCount-${id}`);if(select)fillAutomationSelect(select,candidates,target.deviceId,'Zielgerät wählen',query,count)}
