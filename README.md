...
2. **Only if** — optionally require one or more devices or SALTA functions to match selected states. Up to eight conditions can be configured and they are combined with logical **AND**: every condition must match before the rule can run. The global SALTA **Heating mode** can be used as **Summer mode** or **Winter mode** alongside Presence, Daylight and normal device states. Conditions are evaluated from the current reachable state at execution time.
...
The selectors for trigger, condition and target are searchable. Type any part of the device name, room, source (for example Shelly, Zigbee, Philips Hue, HomeMatic, Presence, Virtual or SALTA), model or logical device type to narrow the list. The internal Heating mode system target is available as an automation condition and action, is intentionally excluded from device triggers, is hidden from the normal device pages and is not published to HomeKit. The editor also shows how many matching targets are currently available.
...
Automation rules are stored in PostgreSQL and restored after restart. Daily time schedules use the additive `automation_time_triggers` persistence table, additional AND conditions use `automation_conditions`, extended device target actions and values use `automation_targets`, and SALTA system actions such as Heating mode use the additive `automation_system_actions` table; the original primary condition, device-trigger and action fields remain for compatibility. References to deleted trigger, condition or target devices are removed automatically by the database. SALTA rejects automation graphs that would create a device-to-device cycle across configured targets. The safe virtual self-reset exception is not treated as a cycle because it explicitly changes the trigger away from the value that fired the rule. If one target command fails at runtime, SALTA logs that target failure and continues with the remaining actions, including the final virtual reset.

The v0.8.x engine currently supports boolean device-state transitions, realtime deCONZ/Zigbee button events, realtime OpenCCU/HomeMatic KEY button events and one daily local-time trigger per rule, together with up to eight AND-linked boolean conditions (including Summer/Winter Heating mode) and up to eight target actions. Time schedules use the configured `TZ` value (default `Europe/Berlin`), so a rule configured for 07:30 continues to run at 07:30 across daylight-saving changes; the repeated autumn clock hour is de-duplicated per local calendar day. A time trigger is currently exclusive and cannot be OR-combined with additional device triggers in the same rule. Targets include binary On/Off/Toggle actions, OpenCCU cover/mode controls, thermostat target temperatures and the global SALTA Heating mode with Summer/Winter actions. Numeric threshold triggers, delays and richer calendar schedules remain planned extensions.
...
- classic HomeMatic `KEY` channels such as the six channels of `HM-PB-6-WM55`, including short press, long press and long-release events for automations.
...
SALTA polls a connected OpenCCU instance every 60 seconds and retries every 15 seconds while the gateway is offline. Transport failures and unusable channel responses invalidate the local JSON-RPC session so SALTA creates a fresh session automatically after an OpenCCU restart. The device catalogue, physical device names and writable VALUES parameter descriptions are refreshed after reconnecting and periodically during normal operation. The parameter descriptions provide the native JSON-RPC value type and write permissions used for switches, dimmers, coverings and thermostats.

Classic HomeMatic `KEY` channels are event-only and return an empty `VALUES` paramset, so SALTA additionally registers a local XML-RPC callback for interfaces that contain such buttons. `BidCos-RF` uses the native OpenCCU XML-RPC port `2001`; SALTA listens on TCP `18099` on the local address used to reach OpenCCU. The OpenCCU host must therefore be able to connect back to the SALTA host on TCP `18099`. `PRESS_SHORT`, `PRESS_LONG` and `PRESS_LONG_RELEASE` are converted into immediate automation button events; repeated `PRESS_CONT` events and `INSTALL_TEST` are deliberately ignored. The callback registration is renewed automatically after an OpenCCU disconnect/reconnect. The existing 60-second polling path remains active for normal device states.

SALTA reuses one JSON-RPC runtime session for polling and commands, serializes OpenCCU operations, and closes the session during reconfiguration, disconnect and controlled shutdown. If OpenCCU returns error 501, SALTA keeps the combined credentials-or-session-limit message visible and pauses scheduled login retries for one minute. SALTA does not manage OpenCCU programs or variables and does not pair HomeMatic devices. Disconnecting OpenCCU removes the synchronized SALTA records but does not delete or reset devices in OpenCCU.
