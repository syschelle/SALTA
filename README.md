...
2. **Only if** — optionally require a second device or the global SALTA **Heating mode** to match a selected state. Heating mode can be **Summer mode** or **Winter mode**. Conditions are evaluated from the current reachable state at execution time.
