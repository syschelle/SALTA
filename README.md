...
For the production image stack, both SALTA and PostgreSQL use host networking. SALTA needs direct host networking so HAP/mDNS can advertise on the Raspberry Pi LAN. PostgreSQL is started explicitly with `listen_addresses=127.0.0.1` and `port=${POSTGRES_HOST_PORT:-5433}`, so the database is reachable only through host loopback and remains unavailable from the LAN. This removes Docker NAT/port-publishing from the database path entirely. The SALTA container drops all Linux capabilities, enables `no-new-privileges`, limits its process count and uses a writable, size-limited `/tmp` with `noexec` and `nosuid`. Application rate limits and login blocks are held in process memory, reset on restart and do not replace firewall or reverse-proxy protection.

See `SECURITY.md` for the exact behavior, configuration guidance and limitations.

## HomeKit

HomeKit is disabled by default and can be enabled at runtime from **Settings → HomeKit** without restarting SALTA. The page shows bridge runtime/advertisement status, pairing state, bridge identity, HAP port, eligible/published device counts and available host network interfaces. Before the bridge is paired, SALTA shows the HomeKit pairing code in the authenticated settings page. The code is hidden after pairing and is never written to application logs. Pairing can be reset deliberately from the same page; this removes the stored HAP pairing identity for the bridge and creates fresh pairing credentials.

The production `docker-compose.image.yml` uses host networking for SALTA so HAP-NodeJS can publish mDNS/Bonjour advertisements directly on the Raspberry Pi LAN. `HOMEKIT_PORT` remains configurable, while the bridge can optionally be bound to a specific host network interface from the HomeKit settings page. PostgreSQL is not exposed to the LAN; it shares the host network namespace but listens only on `127.0.0.1:${POSTGRES_HOST_PORT:-5433}`.

Each compatible device has its own HomeKit section in the device configuration dialog. SALTA controls whether the device is published, allows an optional HomeKit-specific name, and uses the existing SALTA room as the desired HomeKit target room by default. This keeps SALTA as the single room source of truth: room renames and device moves automatically update the stored HomeKit target metadata. An exceptional override can point to another existing SALTA room without maintaining a second room list.
