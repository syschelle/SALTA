...
The v0.8.x engine currently supports boolean device-state transitions and deCONZ/Zigbee button events, together with one optional boolean condition and up to eight target actions. Targets include binary On/Off/Toggle actions, OpenCCU cover/mode controls and thermostat target temperatures. Numeric threshold triggers, time schedules, delays and multiple conditions remain planned extensions.
